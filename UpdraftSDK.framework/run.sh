...
if [ $? = 1 ]; then
    ...
    $PLIST_BUDDY -c "Add :${QUERIES_KEY}:0 string 'dev-updraft'" "${PLIST}"
    $PLIST_BUDDY -c "Add :${QUERIES_KEY}:0 string 'updraft'" "${PLIST}"
else
    if [[ ${QUERIES} =~ updraft ]]; then
        echo "Already exist updraft"
    else
        $PLIST_BUDDY -c "Add :${QUERIES_KEY}:0 string 'dev-updraft'" "${PLIST}"
        $PLIST_BUDDY -c "Add :${QUERIES_KEY}:0 string 'updraft'" "${PLIST}"
    fi
